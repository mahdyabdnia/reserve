...
export default function CreditModal({ className }) {
  ...
  const handleClickButton = () => {
    if (val !== "" && option !== "") {
      alert("در حال انتقال به سایت بانک هستید.");
    }
  };
  const selectOption = [200000, 500000, 10000000];
  const displayOptions = selectOption.map((option) =>
    ...
    <div className={classnames(classes.modal_root, "creditModal")}>
      <div className={classes.modal_content}>
        <div className={classes.tool_bar}>
          <button className={classes.tool_button} onClick={closeModal}>
            <Close />
          </button>
        ...
        <div className={classes.modal_main}>
          ...
          <div className={classes.modal_select_box}>
            {displayOptions.map((i) => {
              return (
                <button
                  className={classes.modal_select_button}
                  onClick={() => {
                    setval(i);
                  }}
                >
          ...
          <button
            className={classes.portal_login_button}
